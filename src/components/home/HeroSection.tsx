...
type SearchFormState = {
  province: string;
  city: string;
  specialty: string; // مقدار انگلیسی که مستقیم به بک‌اند مچ می‌شه
  query: string;
};
...
export default function HeroSection() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (form.query.trim()) params.set("name", form.query.trim());
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-100">
                        ...
                        <div className="mt-2 flex items-center gap-2 text-sm font-black text-emerald-600">
                          <Clock3 size={16} />
                          {featuredDoctor.nextAvailable || "ثبت نشده"}
                        </div>
                      </div>
